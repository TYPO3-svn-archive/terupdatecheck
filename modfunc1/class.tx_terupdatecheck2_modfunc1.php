...
class tx_terupdatecheck2_modfunc1 extends t3lib_extobjbase {
	...
	function main() {
		...
		t3lib_BEfunc::getFuncCheck(0,
		    ...
		    '',
		    '&SET[function]=tx_terupdatecheck2_modfunc1');
		...
		'onclick="document.location=\'index.php?SET[function]=tx_terupdatecheck2_modfunc1&CMD[doUpdate]=1\';return false;" />';

	    if(is_file(PATH_site.'typo3temp/extensions.bin')) {
		$tmp .= ' ('.$LANG->getLL('note_last_update').': '.date('Y-m-d H:i',filemtime(PATH_site.'typo3temp/extensions.bin')).')';
	    }
	...
	function do_update()	{
	    ...
	    if(strpos($msg, 'Error') === false) {
	        ...
	        $tmp .= '<input type="submit" value="'.$LANG->getLL('button_show_ext').'" '.
	                'onclick="document.location=\'index.php?SET[function]=tx_terupdatecheck2_modfunc1\';return false;" />';
	    } else {
...
if (defined("TYPO3_MODE") && $TYPO3_CONF_VARS[TYPO3_MODE]["XCLASS"]["ext/ter_update_check/modfunc1/class.tx_terupdatecheck2_modfunc1.php"])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]["XCLASS"]["ext/ter_update_check/modfunc1/class.tx_terupdatecheck2_modfunc1.php"]);
}
